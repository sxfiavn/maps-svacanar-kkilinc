...
test("correct response for a basic query", async ({ page }) => {
  ...
  await page.route("http://localhost:8080/broadband?state=california&county=alameda", (route) => {
  const mockResponse = {
    result: "success",
...
test("correct response for a basic query with no data", async ({ page }) => {
  ...
  await page.route("http://localhost:8080/broadband?state=florida&county=nodata", (route) => {
    route.fulfill({
      body: JSON.stringify({
        result: "success",
        data: {},
...
test("correct response for an empty query", async ({ page }) => {
  ...
  await page.route("http://localhost:8080/broadband?state=undefined&county=undefined", (route) => {
    route.fulfill({
      body: JSON.stringify({
        result: "success",
        data: {},
...
test("correct response for a half empty query", async ({ page }) => {
  ...
  await page.route("http://localhost:8080/broadband?state=florida&county=undefined", (route) => {
    route.fulfill({
      body: JSON.stringify({
        result: "success",
        data: {},
